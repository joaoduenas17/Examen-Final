...
public class MenuPrincipal extends JFrame implements ActionListener {
	...
	private void crearResultadosEnBarras() {
		...
		for (Prefecto prefecto : prefectos) {
	        dataset.addValue(prefecto.getVotos(), prefecto.getNombre(), "Candidatos");
	    }
		final JFreeChart chart = ChartFactory.createBarChart("Bar Chart", "Candidatos", "Series", dataset,
				PlotOrientation.VERTICAL, true, true, false);
		final ChartPanel chartPanel = new ChartPanel(chart);
		chartPanel.setPreferredSize(new Dimension(600, 400));

		final JInternalFrame frame = new JInternalFrame("Grafico en Barras", true);
	...
	private void crearResultadosEnPastel() {
		...
		final JInternalFrame frame = new JInternalFrame("Grafico en Pastel", true);
